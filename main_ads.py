...
class GameScreen(MDScreen):
    ...
    def _casino_lose(self, *args):
        # У цьому пародійному казино перемога неможлива.
        if self.ad_dialogs:
            dialog = self.ad_dialogs.pop()
            try:
                dialog.dismiss()
    ...
    def show_idiot_ad(self):
        ...
        content.add_widget(MDLabel(
            text="😂 Вітаємо!\\n"
                 "Ви виграли абсолютно нічого.\\n\\n"
                 "Ця реклама настільки рідкісна,\\n"
                 "що краще вже не стане.",
            halign="center",
            valign="middle",
        ))

        # "Пропустити" навмисно створює ще одне вікно.
        dialog = MDDialog(
            title=f"ДУЖЕ РІДКІСНА РЕКЛАМА {self.ad_index}/10",
            type="custom",
            content_cls=content,
            buttons=[
                MDFlatButton(
                    text="ПРОПУСТИТИ РЕКЛАМУ",
                    theme_text_color="Custom",
    ...
    def _duplicate_idiot_ad(self, *args):
        # Кожне натискання "пропустити" дублює рекламу.
        self.show_idiot_ad()
